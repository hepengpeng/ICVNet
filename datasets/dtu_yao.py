...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, ndepths=192, interval_scale=1.06,
                 ...
                 random_crop=False, crop_h=512, crop_w=640, depth_num=64, **kwargs):
        ...
        if img_interp == "linear":
            ...
            self.img_interp = cv2.INTER_LINEAR
        self.random_crop = random_crop  # 是否进行随机裁剪
        self.crop_h = crop_h
        self.crop_w = crop_w
    ...
    def norm_img(self, np_img, self_norm=False, img_mean=None, img_std=None):
        # self_norm指定是否要使用np_img本身的均值和方差进行归一化
        if self_norm:
            # axis=(0,1)参数表示沿着指定轴(0, 1)（即对图像的高和宽）计算相应的均值和方差
            # keepdims=True参数表示保留结果的维度，使得结果仍然具有与原始数据np_img相同的维度。
            ...
            # scale 0~255 to 0~1
            np_img = np_img / 255.
            # 若给定了均值和标准差，则使用提供的均值和标准差参数进行归一化
    ...
    #  crop the image patch with new_h = 512 and new_w = 640 from the center as the training input
    ...
    # The input camera parameters are changed accordingly
    ...
    def getitem_stages(self, idx):
        ...
        else:  # 从邻域帧中随机取(nviews-1)帧,而不是取前(nviews-1)帧
        ...
        for i, vid in enumerate(view_ids):
            ...
            intrinsics = self.crop_cam(intrinsics=intrinsics, h=ori_shape[0] / 2.0, w=ori_shape[1] / 2.0,
                                       new_h=512, new_w=640)
        ...
        sample_interval = depth_range / self.depth_num  # self.depth_num = 64
        sample_depth = []  # 初始stage=1时的64个采样深度
        for i in range(self.depth_num):
            sample_depth.append(np.ones_like(depths[str(0)]) * (sample_interval * (i + i + 1) / 2.0 + depth_min))

        sample_depth = np.stack(sample_depth, axis=0)  # 将list中的4个(H,W)采样深度堆在一起组成一个(4,H,W)大小numpy数组

        imgs = np.stack(imgs).transpose([0, 3, 1, 2])  # Nx(H,W,C) ==> (N,H,W,C) ==> (N,C,H,W)
        proj_matrices = {str(j):np.stack(proj_matrices[str(j)], axis=0) for j in range(stage_num)}  # Nx(4,4)==>(N,4,4)
        cams = {str(j): np.stack(cams[str(j)], axis=0) for j in range(stage_num)}  # Nx(2,4,4) ==> (N,2,4,4)
    ...
    def getitem_stages_random_crop(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                if self.mode == "train":
                    while True:
                        # .item()是将单元素张量转换为标量的方法。
                        #  torch.randint(low=0, high, size) size是指定张量形状的参数
                        rand_start_h = torch.randint(0, h_o - self.crop_h, [1]).item()  # top
                        rand_start_w = torch.randint(0, w_o - self.crop_w, [1]).item()  # left
                        tmp_mask = self.crop_img_any(img=mask_o, start_h=rand_start_h, start_w=rand_start_w, new_h=self.crop_h, new_w=self.crop_w)
                        # begin stages
                        for stage_id in range(stage_num):
                            stage_scale = self.stage_info["scale"][str(stage_id)]
                            stage_mask = self.scale_img(img=tmp_mask, scale=stage_scale, interpolation=cv2.INTER_NEAREST)
                            masks[str(stage_id)] = stage_mask
                        # np.any()检查一个数组中是否存在任意一个元素满足给定条件。
                        # 检查名为masks字典中键为"0"所对应的值是否存在大于0.0的元素,如果存在，则中断当前循环。
            ...
            img = self.crop_img_any(img=img, start_h=rand_start_h, start_w=rand_start_w, new_h=self.crop_h, new_w=self.crop_w)
            intrinsics = self.crop_cam_any(intrinsics=intrinsics, start_h=rand_start_h, start_w=rand_start_w)
            if i == 0:  # reference view
                depth = self.crop_img_any(img=depth, start_h=rand_start_h, start_w=rand_start_w, new_h=self.crop_h, new_w=self.crop_w)
