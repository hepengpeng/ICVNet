...
class MVSDataset(Dataset):
    def __init__(self, datapath, listfile, mode, nviews, 
                 ...
                 max_hw_mode="scale", depth_num=64, out_scales=None, **kwargs):
    ...
    def getitem_stages(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if self.out_scales is not None:  # 例如：out_scales = [2, 3] ==> scale = 2/3
    ...
    def getitem_stages_wo_gt(self, idx):  # without ground truth
        meta = self.metas[idx]
        scan, light_idx, ref_view, src_views = meta
        # use only the reference view and first nviews-1 source views
        view_ids = [ref_view] + src_views[:self.nviews - 1]
    ...
    def __getitem__(self, idx):
        ...
        for i, vid in enumerate(view_ids):
            ...
            if i == 0:  # reference view
                ...
                sample_depth = []
                for i in range(self.depth_num):
                    sample_depth.append(np.ones([ref_img.shape[0], ref_img.shape[1]], dtype=np.float32) *
                                        (sample_interval * (i + i + 1) / 2.0 + depth_min))
                sample_depth = np.stack(sample_depth, axis=0)
