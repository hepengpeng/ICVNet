...
def test_model_stage(model,
    ...
    with torch.no_grad():
        for iteration, sample in enumerate(data_loader):
            ...
            pred_outs = model(sample_cuda, mode="all", max_tree_depth=max_tree_depth, is_clean=is_clean,
                              out_depths=out_depths, prob_out_depths=prob_out_depths)
            ...
            for curr_tree_depth in out_depths:
                prob_maps_prefix_mean["tree_depth_{}".format(curr_tree_depth)] = \
                    prob_maps_prefix["tree_depth_{}".format(curr_tree_depth)] / curr_tree_depth
                
            for curr_tree_depth in out_depths:
                stage_id = depth2stage[str(curr_tree_depth)]
                if stage_id == 0:
                    curr_depth_sample_num = 64
                    next_depth_sample_num = 16
                elif stage_id == 1:
                    curr_depth_sample_num = 16
                    next_depth_sample_num = 4
                elif stage_id == 2:
                    curr_depth_sample_num = 4
                    next_depth_sample_num = 2
                elif stage_id == 3:
                    curr_depth_sample_num = 2
                    next_depth_sample_num = 2
                ...
                for batch_idx in range(batch_size):
                    ...
                    if loss_fn is not None:
                        ...
                        bin_mask = pred_outs["bin_mask_dict"]["tree_depth_{}".format(curr_tree_depth)][batch_idx: batch_idx + 1]
                        ...
                        scalar_outputs["valid_pixel_percentage"] = torch.mean(bin_mask.float())
                        scalar_outputs["abs_depth_error"] = AbsDepthError_metrics(depth_est, depth_gt, mask > 0.0)

                        scalar_outputs["accu"] = ICVNet_accu(pred_label, gt_label, mask > 0.0)
                        for i in range(curr_depth_sample_num):
                            scalar_outputs["accu{}".format(i)] = ICVNet_accu(pred_label, gt_label,
                                torch.logical_and(torch.eq(gt_label, i), mask > 0.0))

                        scalar_outputs["thres0.125mm_error"] = Thres_metrics(depth_est, depth_gt, mask > 0.0, 0.125)
                        scalar_outputs["thres0.25mm_error"] = Thres_metrics(depth_est, depth_gt, mask > 0.0, 0.25)
                        scalar_outputs["thres0.5mm_error"] = Thres_metrics(depth_est, depth_gt, mask > 0.0, 0.5)
                        scalar_outputs["thres1mm_error"] = Thres_metrics(depth_est, depth_gt, mask > 0.0, 1)
                        ...
                        scalar_outputs["thres0.125mm_accu"] = 1.0 - scalar_outputs["thres0.125mm_error"]
                        scalar_outputs["thres0.25mm_accu"] = 1.0 - scalar_outputs["thres0.25mm_error"]
                        scalar_outputs["thres0.5mm_accu"] = 1.0 - scalar_outputs["thres0.5mm_error"]
                        scalar_outputs["thres1mm_accu"] = 1.0 - scalar_outputs["thres1mm_error"]
                        ...
                        scalar_outputs["front_prob"] = \
                            Prob_mean(prob_map, sample_cuda["masks"][str(depth2stage[str(max_tree_depth)])][batch_idx:batch_idx + 1] > 0.0)
                        scalar_outputs["back_prob"] = \
                            Prob_mean(prob_map, sample_cuda["masks"][str(depth2stage[str(max_tree_depth)])][batch_idx:batch_idx + 1] == 0.0)
                        ...
                        logger.info(
                            " ".join(
                                [
                                    ...
                                    "valid_pixel_percentage {:.4f}".format(scalar_outputs["valid_pixel_percentage"]),
                                    "abs_depth_error {:.4f}".format(scalar_outputs["abs_depth_error"]),
                                    "thres0.125mm_error {:.4f}".format(scalar_outputs["thres0.125mm_error"]),
                                    "thres0.25mm_error {:.4f}".format(scalar_outputs["thres0.25mm_error"]),
                                    "thres0.5mm_error {:.4f}".format(scalar_outputs["thres0.5mm_error"]),
                                    "thres1mm_error {:.4f}".format(scalar_outputs["thres1mm_error"]),
                                    "thres2mm_error {:.4f}".format(scalar_outputs["thres2mm_error"]),
                                    "thres4mm_error {:.4f}".format(scalar_outputs["thres4mm_error"]),
                                    "thres0.125mm_accu {:.4f}".format(scalar_outputs["thres0.125mm_accu"]),
                                    "thres0.25mm_accu {:.4f}".format(scalar_outputs["thres0.25mm_accu"]),
                                    "thres0.5mm_accu {:.4f}".format(scalar_outputs["thres0.5mm_accu"]),
                                    "thres1mm_accu {:.4f}".format(scalar_outputs["thres1mm_accu"]),
    ...
    with torch.no_grad():
        for iteration, sample in enumerate(data_loader):
            ...
            for batch_idx in range(len(scan_names)):
                scan_name = scan_names[batch_idx]
                img_id = img_ids[batch_idx]
                test_scalars = {"forward_time": forward_time, "data_time": data_time,
                                "max_mem": forward_max_memory_allocated}
...
def test(rank, cfg):
    ...
    torch.distributed.init_process_group(backend="gloo", rank=rank, world_size=cfg["world_size"])
    synchronize()
    set_random_seed(cfg["random_seed"])
    logger = setup_logger("icvnet_test{}".format(str(rank)), cfg["log_dir"], prefix="test")
    ...
    test_data_loader = DataLoader(test_dataset, cfg["test"]["batch_size"], sampler=test_sampler,
    ...
    if cfg.get("test_func_name", "test_model_stage") == "test_model_stage":
        avg_test_scalars, all_test_scalars_dict = test_model_stage(model, model_loss,
            ...
            my_rank=rank,
            logger=logging.getLogger("icvnet_test{}".format(str(rank)) + ".test"),
            color_mode=cfg["data"]["test"]["color_mode"],
...
def gipuma_filter(cfg):
    ...
    for curr_tree_depth in save_depths:
        ...
        for para_id in range(len(cfg["fusion"]["prob_filter"])):
            ...
            collect_args.target_dir = osp.join(cfg["output_dir"], "depth_{}".format(curr_tree_depth),
                "gipuma_filter"+target_suffix, "collected_points_{}".format(prob_threshold)) \
                if prob_filter else osp.join(cfg["output_dir"], "depth_{}".format(curr_tree_depth),
                                             "gipuma_filter"+target_suffix, "collected_points")
    ...
    for curr_tree_depth in save_depths:
        ...
        for para_id in range(cfg["fusion"]["gipuma_filter_per"]["para_num"]):
            for scan in scans:
                ...
                dense_folder = os.path.join(depth_data_folder, scan)
                point_folder = fusion_one(dense_folder=dense_folder,
                                          fusibile_exe_path=cfg["fusion"]["gipuma_filter_per"]["fusibile_exe_path"],
                                          prob_filter=prob_filter, prob_threshold=prob_threshold,
                                          disp_threshold=disp_threshold, num_consistent=num_consistent)
            collect_args = type('tmp', (object,), {})()
            collect_args.target_dir = osp.join(cfg["output_dir"], "depth_{}".format(curr_tree_depth),
                "gipuma_filter_per", "collected_points_{}".format(prob_threshold)) \
                if prob_filter else osp.join(cfg["output_dir"], "depth_{}".format(curr_tree_depth),
                                             "gipuma_filter_per", "collected_points")
...
def xy_filter(rank, cfg):
    ...
    for curr_tree_depth in save_depths:
        ...
        for para_id in range(len(cfg["fusion"]["xy_filter"]["prob_threshold"])):
            ...
            img_dist_thresh = cfg["fusion"]["xy_filter"]["img_dist_thresh"][para_id] \
                if cfg["fusion"]["xy_filter"].get("img_dist_thresh", None) is not None else 1.0
            depth_thresh = cfg["fusion"]["xy_filter"]["depth_thresh"][para_id] \
                if cfg["fusion"]["xy_filter"].get("depth_thresh", None) is not None else 0.01

            point_dir = os.path.join(output_dir, str(para_id), "depth_{}".format(curr_tree_depth),
                                     "xy_filter", "collected_points_{}".format(prob_threshold)) \
                if prob_threshold != 0.0 else os.path.join(output_dir, str(para_id), "depth_{}".format(curr_tree_depth),
                                                           "xy_filter", "collected_points")
            ...
            for scan in scans:
                ...
                filter_depth(scan_folder=scan_folder, pair_path=pair_path, plyfilename=ply_name,
                             prob_threshold=prob_threshold, num_consistent=num_consistent,
                             img_dist_thresh=img_dist_thresh, depth_thresh=depth_thresh)
    ...
    for curr_tree_depth in save_depths:
        ...
        for para_id in range(cfg["fusion"]["xy_filter_per"]["para_num"]):
            ...
            for scan in scans:
                ...
                point_dir = os.path.join(output_dir, str(para_tag), "depth_{}".format(curr_tree_depth),
                                         "xy_filter_per", "collected_points_{}".format(prob_threshold)) \
                    if prob_threshold is not None else os.path.join(output_dir, str(para_tag),
                                                                    "depth_{}".format(curr_tree_depth),
                                                                    "xy_filter_per", "collected_points")
...
def main():
    parser = argparse.ArgumentParser(description="PyTorch ICVNet Testing")
    parser.add_argument("--cfg", dest="config_file", default="configs/test_dtu.yaml", metavar="FILE",
    ...
    logger = setup_logger("icvnet", log_dir, prefix="test")
