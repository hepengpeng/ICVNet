...
def train_model_stage(model,
    ...
    for iteration, sample in enumerate(data_loader):
        ...
        for curr_tree_depth in range(1, max_tree_depth + 1):
            ...
            gt_label, bin_mask = depth_update.get_gt_label_bin_edge(depth_gt,
                b_tree=sample_cuda["binary_tree"]["tree"], depth_start=depth_min_max[:, 0],
                depth_end=depth_min_max[:, 1], stage_id=stage_id)
            ...
            with torch.no_grad():
                if (bin_mask_prefix.shape[1] != bin_mask.shape[1]) or (bin_mask_prefix.shape[2] != bin_mask.shape[2]):
                    bin_mask_prefix = torch.squeeze(F.interpolate(torch.unsqueeze(bin_mask_prefix.float(), 1),
                                                    [bin_mask.shape[1], bin_mask.shape[2]], mode="nearest"), 1).bool()
            ...
            sample_cuda["binary_tree"]["depth"], sample_cuda["binary_tree"]["tree"] = \
                depth_update.update_tree_with_pred_label(sample_cuda["binary_tree"]["tree"],
                                                         torch.unsqueeze(pred_label, 1),
                                                         depth_start=depth_min_max[:, 0],
                                                         depth_end=depth_min_max[:, 1],
                                                         stage_id=stage_id)

            if stage_id == 0:
                depth_est = (sample_cuda["binary_tree"]["depth"][:, 7] + sample_cuda["binary_tree"]["depth"][:, 8]) / 2.0
            elif stage_id == 1:
                depth_est = (sample_cuda["binary_tree"]["depth"][:, 1] + sample_cuda["binary_tree"]["depth"][:, 2]) / 2.0
            else:
                depth_est = (sample_cuda["binary_tree"]["depth"][:, 0] + sample_cuda["binary_tree"]["depth"][:, 1]) / 2.0

            next_depth_stage = depth2stage[str(curr_tree_depth + 1)]
            if next_depth_stage != stage_id:
                sample_cuda["binary_tree"]["depth"], sample_cuda["binary_tree"]["tree"] = \
                    depth_update.update_tree_with_depthmap(depth_est, curr_tree_depth + 1,
                                                           depth_start=depth_min_max[:, 0],
                                                           depth_end=depth_min_max[:, 1],
                                                           stage_id=stage_id, scale_factor=2.0,
                                                           mode='bilinear')
            ...
            image_outputs = {"depth_est": depth_est_mapped, "depth_gt": depth_gt_mapped,
                             ...
                             "mask": mask.cpu(), "ori_mask": sample["masks"][str(stage_id)],
    ...
    if tensorboard_logger is not None and my_rank == 0:
        for curr_tree_depth in range(1, max_tree_depth + 1):
            for key, value in avg_train_scalars["depth{}".format(curr_tree_depth)].mean().items():
                tensorboard_logger.add_scalars("Avg_" + key + "_depth{}".format(curr_tree_depth),
                                               {"train": value}, curr_epoch)
            logger.info("Epoch {} depth {} average train scalars {}".format(curr_epoch, curr_tree_depth,
                        avg_train_scalars["depth{}".format(curr_tree_depth)].mean()))
        ...
        if tensorboard_logger is not None and my_rank == 0:
            for curr_tree_depth in range(1, max_tree_depth + 1):
                for key, value in avg_test_scalars["depth{}".format(curr_tree_depth)].mean().items():
                    tensorboard_logger.add_scalars("Avg_" + key + "_depth{}".format(curr_tree_depth),
                                                   {"val": value}, curr_epoch)
                logger.info("Epoch {} depth {} average val scalars {}".format(curr_epoch, curr_tree_depth,
                            avg_test_scalars["depth{}".format(curr_tree_depth)].mean()))
...
def train(rank, cfg):
    if cfg.get("slurm", False):
        ...
        logger = setup_logger("icvnet_train{}".format(str(slurm_local_rank)), cfg["log_dir"], prefix="train")
        ...
        synchronize()
        logger = setup_logger("icvnet_train{}".format(str(rank)), cfg["log_dir"], prefix="train")
    ...
    if cfg.get("slurm", False):
        model = DistributedDataParallel(model, device_ids=[slurm_local_rank],
                                        ...
                                        broadcast_buffers=cfg.get("broadcast_buffers", True))
    else:
    ...
    train_dataset = MVSDataset(cfg["data"]["train"]["root_dir"],
        ...
        depth_num=cfg["data"]["train"].get("depth_num", 64),
    ...
    val_dataset = ValMVSDataset(cfg["data"]["val"]["root_dir"],
        ...
        depth_num=cfg["data"]["val"].get("depth_num", 64))

    if cfg.get("slurm", False):
        rank = slurm_global_rank

    train_sampler = DistributedSampler(train_dataset, num_replicas=world_size, rank=rank, shuffle=True)
    train_data_loader = DataLoader(train_dataset, cfg["train"]["batch_size"], sampler=train_sampler,
                                   num_workers=cfg["data"]["num_workers"])

    val_sampler = DistributedSampler(val_dataset, num_replicas=world_size, rank=rank, shuffle=False)
    val_data_loader = DataLoader(val_dataset, cfg["val"]["batch_size"], sampler=val_sampler,
                                 num_workers=cfg["data"]["num_workers"])
    ...
    for epoch in range(start_epoch, max_epoch):
        ...
        init_max_depth = cfg["model"].get("init_max_depth", 1)
        if epoch < 6:
            max_tree_depth = min(epoch // 2 + init_max_depth, max_max_tree_depth)
        else:
            max_tree_depth = max_max_tree_depth
        ...
        train_func(model,
                   ...
                   curr_epoch=epoch,
                   my_rank=rank,
                   logger=logging.getLogger("icvnet_train{}".format(str(rank)) + ".train"),
                   log_period=cfg["train"]["log_period"],
                   tensorboard_logger=tensorboard_logger)

        if cfg["scheduler"]["name"] == 'multi_step':
            # The whole network is optimized by Adam optimizer in Pytorch for 16 epochs with an initial
            # learning rate of 0.0001, which is down-scaled by a factor of 2 after 10, 12, and 14 epochs.
            scheduler.step()
        ...
        if epoch % val_period == 0 or (epoch + 1) == max_epoch:
            avg_test_scalar = validate_model_stage(model,
                                                   ...
                                                   curr_epoch=epoch,
                                                   my_rank=rank,
                                                   logger=logging.getLogger("icvnet_train{}".format(str(rank))+".val"),
...
def main():
    parser = argparse.ArgumentParser(description="PyTorch ICVNet Training")
    parser.add_argument("--cfg", dest="config_file", default="configs/train_dtu.yaml", metavar="FILE",
    ...
    logger = setup_logger("icvnet", log_dir, prefix="train")
