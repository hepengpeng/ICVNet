...
class Conv2dUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1, relu=True, norm="batchnorm",
                 bn_momentum=0.1, in_momentum=0.1, in_affine=True, num_groups=8, **kwargs):
        super(Conv2dUnit, self).__init__()
        bias = False
        if norm is None:
            bias = True
        self.conv = nn.Conv2d(in_channels, out_channels, kernel_size, stride=stride, bias=bias, **kwargs)
...
class Deconv2dUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size, stride=1,
        ...
        if norm is None:
            bias = True
        self.conv = nn.ConvTranspose2d(in_channels, out_channels, kernel_size, stride=stride, bias=bias, **kwargs)
        self.norm_layer = None
        if norm == "batchnorm":
            self.norm_layer = nn.BatchNorm2d(out_channels, momentum=bn_momentum)
        if norm == "groupnorm":
...
class Conv3dUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.conv = nn.Conv3d(in_channels, out_channels, kernel_size, stride=stride, bias=bias, **kwargs)
...
class Deconv3dUnit(nn.Module):
    ...
    def __init__(self, in_channels, out_channels, kernel_size=3, stride=1,
        ...
        self.conv = nn.ConvTranspose3d(in_channels, out_channels, kernel_size, stride=stride, bias=bias, **kwargs)
...
class Deconv2dBlock(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size, relu=True, norm="batchnorm", bn_momentum=0.1,
                 in_momentum=0.1, in_affine=True, num_groups=8):
...
class StageFeatExtNet(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        if output_channels is None:
            ...
            self.output_channels = output_channels  # output_channels：[64, 32, 16, 8]
...
class DeformStageNet(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        if stage_num == 4:
            ...
            self.conv_out["1"] = DeformConv2dPack(base_channels * 8, self.output_channels[1],
                                                  kernel_size=3, stride=1, padding=1, deform_groups=1)
            self.conv_out["2"] = DeformConv2dPack(base_channels * 4, self.output_channels[2],
                                                  kernel_size=3, stride=1, padding=1, deform_groups=1)
            self.conv_out["3"] = DeformConv2dPack(base_channels * 2, self.output_channels[3],
                                                  kernel_size=3, stride=1, padding=1, deform_groups=1)
...
class CostRegNet(nn.Module):
    def __init__(self, in_channels, base_channels, out_channels=1):
        ...
        self.deconv7 = Deconv3dUnit(base_channels * 8, base_channels * 4, stride=(1, 2, 2), padding=1,
                                    output_padding=(0, 1, 1), norm="groupnorm")

        self.deconv8 = Deconv3dUnit(base_channels * 4, base_channels * 2, stride=(1, 2, 2), padding=1,
                                    output_padding=(0, 1, 1), norm="groupnorm")

        self.deconv9 = Deconv3dUnit(base_channels * 2, base_channels * 1, stride=(1, 2, 2), padding=1,
                                    output_padding=(0, 1, 1), norm="groupnorm")
...
class CostRegNetBN(nn.Module):
    def __init__(self, in_channels, base_channels, out_channels=1):
        ...
        self.deconv7 = Deconv3dUnit(base_channels * 8, base_channels * 4, stride=(1, 2, 2), padding=1,
                                    output_padding=(0, 1, 1), norm="batchnorm")
...
class FeatureFetcher(nn.Module):
    ...
    def forward(self, feature_maps, pts, cam_intrinsics, cam_extrinsics):
        ...
        with torch.no_grad():
            ...
            normal_uv = torch.cat([torch.div(x, z).unsqueeze(-1), torch.div(y, z).unsqueeze(-1),
                                   torch.ones_like(x).unsqueeze(-1)], dim=-1)
