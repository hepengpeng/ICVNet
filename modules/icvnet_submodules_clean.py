...
class DeformStageNet(nn.Module):
    def __init__(self, base_channels, stage_num=4, output_channels=None):
        ...
        if stage_num == 4:
            ...
            self.conv_out["1"] = DeformConv2dPack(base_channels * 8, self.output_channels[1], kernel_size=3,
                                                  stride=1, padding=1, deform_groups=1)
            self.conv_out["2"] = DeformConv2dPack(base_channels * 4, self.output_channels[2], kernel_size=3,
                                                  stride=1, padding=1, deform_groups=1)
            self.conv_out["3"] = DeformConv2dPack(base_channels * 2, self.output_channels[3], kernel_size=3,
                                                  stride=1, padding=1, deform_groups=1)
