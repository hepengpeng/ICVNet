...
def check_geometric_consistency(depth_ref, intrinsics_ref, extrinsics_ref, depth_src, intrinsics_src, extrinsics_src, img_dist_thresh=1.0, depth_thresh=0.01):
    width, height = depth_ref.shape[1], depth_ref.shape[0]
    x_ref, y_ref = np.meshgrid(np.arange(0, width), np.arange(0, height))
...
def filter_depth(scan_folder, pair_path, plyfilename, prob_threshold=0.8, num_consistent=3, img_dist_thresh=1.0, depth_thresh=0.01):
    # the pair file
    pair_file = os.path.join(pair_path)
    ...
    for ref_view, src_views in pair_data:
        ...
        if prob_threshold == 0.0:
            ...
            if photo_mask.shape[0] != ref_depth_est.shape[0] or photo_mask.shape[1] != ref_depth_est.shape[1]:
                ...
                photo_mask = torch.squeeze(
                    F.interpolate(
                        torch.unsqueeze(torch.unsqueeze(photo_mask_t, 0), 0), 
        ...
        save_mask(os.path.join(scan_folder, "mask/{:0>8}_geo.png".format(ref_view)), geo_mask)
        save_mask(os.path.join(scan_folder, "mask/{:0>8}_final.png".format(ref_view)), final_mask)
    ...
    # Improvement II
    # vertexs = np.array([tuple(v) for v in vertexs], dtype=[('x', 'f4'), ('y', 'f4'), ('z', 'f4')])  # 这步运行很慢,提高效率
    # vertex_colors = np.array([tuple(v) for v in vertex_colors], dtype=[('red', 'u1'), ('green', 'u1'), ('blue', 'u1')])  # 这步运行很慢,提高效率

    # Improvement II
    vertexs_dtype = [('x', 'f4'), ('y', 'f4'), ('z', 'f4')]
    new_vertexs = np.empty(vertexs.shape[0], dtype=vertexs_dtype)
    new_vertexs['x'], new_vertexs['y'], new_vertexs['z'] = vertexs[:, 0], vertexs[:, 1], vertexs[:, 2]
    vertexs = new_vertexs
    del new_vertexs
    vertex_colors_dtype = [('red', 'u1'), ('green', 'u1'), ('blue', 'u1')]
    new_vertex_colors = np.empty(vertex_colors.shape[0], dtype=vertex_colors_dtype)
    new_vertex_colors['red'], new_vertex_colors['green'], new_vertex_colors['blue'] = \
        vertex_colors[:, 0], vertex_colors[:, 1], vertex_colors[:, 2]
    vertex_colors = new_vertex_colors
    del new_vertex_colors
