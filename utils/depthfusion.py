...
from datasets.data_io import load_cam, read_pfm, save_pfm
...
def mvsnet_to_gipuma(dense_folder, gipuma_point_folder, depth_suffix='_init.pfm'):
    
    image_folder = dense_folder
    cam_folder = dense_folder
    depth_folder = dense_folder
    ...
    image_names = [i for i in image_names if os.path.splitext(i)[1] == ".jpg"]
    for image_name in image_names:
        image_prefix = os.path.splitext(image_name)[0]
        in_cam_file = os.path.join(depth_folder, "cam_" + image_prefix+'_init.txt')
    ...
    for image_name in image_names:
        ...
        in_depth_pfm = os.path.join(depth_folder, image_prefix+depth_suffix)
...
def probability_filter(dense_folder, prob_threshold):
    ...
    for image_name in image_names:
        ...
        out_depth_map_path = os.path.join(depth_folder, image_prefix+'_prob_filtered_{}.pfm'.format(prob_threshold))

        depth_map = read_pfm(init_depth_map_path)[0]
        prob_map = read_pfm(prob_map_path)[0]
        depth_map[prob_map < prob_threshold] = 0
        save_pfm(out_depth_map_path, depth_map)        
        out_depth_img_path = os.path.join(depth_folder, image_prefix+'_prob_filtered_{}.png'.format(prob_threshold))
        plt.imsave(out_depth_img_path, depth_map, cmap="rainbow")
...
def fusion_one(dense_folder, fusibile_exe_path, prob_filter=False, prob_threshold=0.8, disp_threshold=0.25, num_consistent=3):

    point_folder = os.path.join(dense_folder, 'points_mvsnet')
    if prob_filter == True:
        point_folder += "_prob_{}".format(prob_threshold)
    ...
    if prob_filter == True:
        print ('Convert mvsnet output to gipuma input, ' + "depth_suffix=_prob_filtered_{}.pfm".format(prob_threshold))
        mvsnet_to_gipuma(dense_folder, point_folder, '_prob_filtered_{}.pfm'.format(prob_threshold))
    else:
        print ('Convert mvsnet output to gipuma input, ' + "depth_suffix=_init.pfm")
...
if __name__ == '__main__':
    ...
    parser.add_argument('--fusibile_exe_path', type=str, default='/ssddata/zmiaa/mvs-net/YaoYao/fusibile/build/fusibile')
    parser.add_argument('--disp_threshold', type=float, default='0.25')
    parser.add_argument('--num_consistent', type=float, default='3')
    parser.add_argument('--scene_names', nargs='+', default=None)
    parser.add_argument('--prob_filter', default=False, action='store_true')
    ...
    if args.scene_names is not None:
        ...
        scene_names = [i for i in scene_names if os.path.isdir(os.path.join(data_folder, i))]

    for scene_name in scene_names:
        dense_folder = os.path.join(data_folder, scene_name)
        fusion_one(dense_folder, fusibile_exe_path, prob_filter, prob_threshold, disp_threshold, num_consistent)