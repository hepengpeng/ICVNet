...
def update_tree(binary_tree, indicator, direction):
    """update the tree to the next or last level, or keep to the current level
    Args:
        indicator (integer tensor): indicator tensor for updating.
            -1 means going up and 1 means going down, while 0 means staying.
    ...
    tree_key = torch.clamp_min((binary_tree[:, 1, :, :] * (2.0 ** torch.squeeze(indicator, 1))).type(torch.int64) +
                               torch.squeeze(direction, 1), 0)
    tree_key = torch.minimum(tree_key, 2 ** (tree_list[0] + 4) - 1)
